...
export default function _Table({ module, open }) {
	...
	const handleEntityEdit = (id) => {
		...
		dispatch(
			editEntityData({
				url: `${MASTER_DATA_ROUTES.API_ROUTES.SETTING.VIEW}/${id}`,
				module,
			})
		);
		navigate(`${MASTER_DATA_ROUTES.NAVIGATION_LINKS.SETTING.INDEX}/${id}`);
	};
	...
	const handleDeleteSuccess = async (id) => {
		const res = await dispatch(
			deleteEntityData({
				url: `${MASTER_DATA_ROUTES.API_ROUTES.SETTING.DELETE}/${id}`,
				module,
				id,
			})
		);
		...
		if (deleteEntityData.fulfilled.match(res)) {
			...
			navigate(MASTER_DATA_ROUTES.NAVIGATION_LINKS.SETTING);
