...
import { Button, rem, Flex, Grid, Box, ScrollArea, Text, Title, Stack, Center, Checkbox } from "@mantine/core";
...
function _SettingsForm(props) {
	...
	const [setFormDataForUpdate] = useState(false);
					...
					<Box bg={"white"} p={"xs"} className={"borderRadiusAll"} mb={"8"}>
						<Box bg={"white"}>
							...
							<Box pl={`xs`} pr={"xs"} className={"borderRadiusAll"}>
								<ScrollArea h={height + 40} scrollbarSize={2} scrollbars="y" type="never">
									<Box pt={"xs"} pl={"xs"}>
										<Box mt={"xs"}>
											<Grid gutter={{ base: 1 }}>
												<Grid.Col span={2}>
